...
def random_mild_fir(rng: np.random.Generator, length: int, sigma: float = 0.05) -> np.ndarray:
    """Generate a small random FIR for "extra memory" mismatch.

    - h[0] is near 1
    - remaining taps are small complex values
    """
    L = int(length)
    if L <= 1:
    ...
    noise = (rng.standard_normal(L - 1) + 1j * rng.standard_normal(L - 1)) / np.sqrt(2.0)
    h[1:] = sigma * decay * noise
    return h


class PAMismatchWrapper:
    """Wrap a PA model and inject per-branch mismatches.

    This is a *behavioral* mismatch injection for array simulations.
    It is intentionally simple and works with any PA object that has `forward(x)`.

    Injected effects
    ---------------
    - AM/AM (drive_db, gain_db):
        * drive_db : input scaling (different effective backoff)
        * gain_db  : output gain scaling
    - AM/PM (ampm_deg):
        amplitude-dependent phase rotation using normalized instantaneous power
    - Memory difference (lin_mem_fir):
        a small linear FIR at the output (optional) to emulate per-branch memory diversity
    """
    ...
    def __init__(
        ...
        phase_deg: float = 0.0,
        ampm_deg: float = 0.0,
        lin_mem_fir: np.ndarray | None = None,
    ):
        self.pa = pa
        self.drive = _db_to_lin(drive_db)
        self.gain = _db_to_lin(gain_db) * np.exp(1j * np.deg2rad(float(phase_deg)))
        self.ampm = np.deg2rad(float(ampm_deg))
        self.lin_mem_fir = None if lin_mem_fir is None else np.asarray(lin_mem_fir).reshape(-1).astype(complex)

    def forward(self, x: np.ndarray) -> np.ndarray:
        x = np.asarray(x).reshape(-1)
        xd = self.drive * x
        y = self.pa.forward(xd)

        # AM/PM: phase = k * (|x|^2/mean(|x|^2) - 1)
        if self.ampm != 0.0:
            p = np.abs(xd) ** 2
            p_mean = float(np.mean(p)) + 1e-12
            p_norm = p / p_mean
            phase = self.ampm * (p_norm - 1.0)
            y = y * np.exp(1j * phase)

        y = self.gain * y

        if self.lin_mem_fir is not None and len(self.lin_mem_fir) > 1:
            y = np.convolve(self.lin_mem_fir, y)[: len(y)]
        return y
